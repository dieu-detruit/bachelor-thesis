...
ビームをシャッターで遮り、光源からの光が入射していない状態で撮影しダークフレームを取得すると、それは一様に0とはならず、ノイズが存在する状態が確認できる。
これは主に、CCDカメラに壊れている素子が存在していること、実験室を完全に暗転できないこと、熱による暗電流の3つが影響していると考えられ、位相回復に用いる計測データはこのノイズ成分を除去したものであることが望ましい。
そこで、計測実験ごとに計測開始の直前に計8枚のダークフレームを取得し、単純加算平均の結果を計測データから差し引いた。
露光時間は計測実験と同じ値に設定している。
